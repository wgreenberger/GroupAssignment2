...
Stocks <- tq_get(c("AAPL", "MSFT", "GOOG", "AMZN"), from = start, to = end)
...
ui <- dashboardPage(
  dashboardHeader(title = "Stock Price Data", inverted = TRUE),
  dashboardSidebar(
...
#names(table(SYMBOLS$Name))[input$select]
 # })
  
  #stock_ticker <- reactive({
  
  #SYMBOLS$Symbol[which(SYMBOLS$Name == Selected_stock)]
  #})
  
  
  Stocks2 <- reactive({
    Stocks21 <- as.tibble(tq_get((SYMBOLS$Symbol[which(SYMBOLS$Name ==  names(table(SYMBOLS$Name))[input$select])]), from = start, to = end))
    print(Stocks21)
    Stocks21
  })
  ...
  output$dotplot1 <- renderPlotly({
    ggplotly(ggplot(Stocks2(), aes_string(x = date, y = close)) + geom_point(aes_string(date, close, color = symbol)))
  })
